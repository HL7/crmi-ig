...
* **artifactCommentType**: If a comment is submitted as part of the approval, this parameter denotes the type of artifact comment (and must belong to the [Artifact Assessment Information Type ValueSet](http://hl7.org/fhir/ValueSet/artifactassessment-information-type) ValueSet).
* **artifactCommentSummary**: If a comment is submitted as part of the approval, this parameter contains the body of the comment.
...
* **artifactCommentRelatedArtifact**: Optional supporting reference for the comment.
* **artifactCommentAuthor**: A Reference to the party approving the artifact for release. The content of the referenced resource must match the data which is specified as part of the endorser parameter.
